...
function Post(props: { onClickPackage?: () => void, display?: string }) {
  const diplay = props.display;
  return (
    <div className={styles.postContainer}>
      <ProfileImage w={'100%'} br={5} alt='contentPic' imgUrl={user} />
      <div className={styles.profileInfo} style={{ display: diplay && diplay }}>
        <div className={styles.profileImage}>
          <ProfileImage w={40} h={40} br={'50%'} alt='sellerProPic' imgUrl={user} />
        </div>
        <span className={styles.userName} style={{ display: diplay && diplay }}>Kushan Madusha, <span className={styles.location}>Polonnaruwa</span></span>
      </div>
      ...
      <div className={styles.dateInfo}>
        ...
        <span className={styles.packages} style={{ display: diplay && diplay }} onClick={props.onClickPackage}>see packages</span>
      </div>
    </div>
  );
}
